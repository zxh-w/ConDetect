...
func (b *BaseApi) LoadDashboardBaseInfo(c *gin.Context) {
	...
	// 还有imageNumber,containerNumber,macilousNumber,VulnNUmber
	imageTages, err := imageService.List()
	if err != nil {
		helper.ErrorWithDetail(c, constant.CodeErrInternalServer, constant.ErrTypeInternalServer, err)
		return
	}
	data.ImageNumber = len(imageTages)
	ContainerList, err := containerService.List()
	if err != nil {
		helper.ErrorWithDetail(c, constant.CodeErrInternalServer, constant.ErrTypeInternalServer, err)
		return
	}
	data.ContainerNumber = len(ContainerList)
	helper.SuccessWithData(c, data)
}
