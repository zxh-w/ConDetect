...
// @Description 修改APP使能
// @Accept json
// @Param request body dto.AppManage true "request"
// @Success 200
// @Security ApiKeyAuth
// @Router /containers/app/update/enable [post]
// @x-panel-log {"bodyKeys":["name","container"],"paramKeys":[],"BeforeFunctions":[],"formatZH":"修改容器 [container] APP [name]的使能","formatEN":"update container app [name]"}
func (b *BaseApi) UpdateAppEnable(c *gin.Context) {
	var req dto.AppManage
	if err := helper.CheckBindAndValidate(&req, c); err != nil {
		return
	}

	if err := containerService.UpdateAppEnable(req); err != nil {
...
// @Success 200
// @Security ApiKeyAuth
// @Router /containers/app/update/status [post]
// @x-panel-log {"bodyKeys":["name","container"],"paramKeys":[],"BeforeFunctions":[],"formatZH":"修改容器 [container] APP [name]的状态","formatEN":"update container app [name]"}
func (b *BaseApi) UpdateAppStatus(c *gin.Context) {
	var req dto.AppManage
	if err := helper.CheckBindAndValidate(&req, c); err != nil {
		return
	}

	if err := containerService.UpdateAppStatus(req); err != nil {
		helper.ErrorWithDetail(c, constant.CodeErrInternalServer, constant.ErrTypeInternalServer, err)
		return
	}
	helper.SuccessWithData(c, nil)
...
// @Success 200
// @Security ApiKeyAuth
// @Router /containers/app/uninstall [post]
// @x-panel-log {"bodyKeys":["name"],"paramKeys":[],"BeforeFunctions":[],"formatZH":"卸载容器APP [name]","formatEN":"uninstall container app [name]"}
func (b *BaseApi) UninstallAppManage(c *gin.Context) {
	var req dto.AppManage
	if err := helper.CheckBindAndValidate(&req, c); err != nil {
		return
	}

	if err := containerService.UninstallApp(req); err != nil {
		helper.ErrorWithDetail(c, constant.CodeErrInternalServer, constant.ErrTypeInternalServer, err)
		return
	}
	helper.SuccessWithData(c, nil)
...
func (b *BaseApi) SearchAppManage(c *gin.Context) {
	var req dto.AppSearch
	if err := helper.CheckBindAndValidate(&req, c); err != nil {
		return
	}
	_, list, err := containerService.SearchAppWithContainer(req)
	if err != nil {
		helper.ErrorWithDetail(c, constant.CodeErrInternalServer, constant.ErrTypeInternalServer, err)
		return
	}
