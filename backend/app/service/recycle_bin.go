...
func (r RecycleBinService) Page(search dto.PageInfo) (int64, []response.RecycleBinDTO, error) {
	...
	for _, p := range partitions {
		dir := path.Join(p.Mountpoint, ".condetect_clash")
		if !op.Stat(dir) {
			continue
		}
...
func (r RecycleBinService) Clear() error {
	...
	for _, p := range partitions {
		...
		newDir := path.Join(p.Mountpoint, "condetect_clash")
...
func getClashDir(realPath string) (string, error) {
	...
	for _, p := range partitions {
		...
		if strings.HasPrefix(realPath, p.Mountpoint) {
			clashDir := path.Join(p.Mountpoint, ".condetect_clash")
			if err = createClashDir(path.Join(p.Mountpoint, ".condetect_clash")); err != nil {
				return "", err
			}
			return clashDir, nil
		}
