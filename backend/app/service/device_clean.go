...
import (
	...
	fileUtils "ConDetect/backend/utils/files"

	"github.com/google/uuid"
)

const (
	upgradePath      = "condetect/tmp/upgrade"
	snapshotTmpPath  = "condetect/tmp/system"
	rollbackPath     = "condetect/tmp"
	cachePath        = "condetect/cache"
	oldOriginalPath  = "original"
	oldAppBackupPath = "condetect/resource/apps_bak"
	oldDownloadPath  = "condetect/tmp/download"
	oldUpgradePath   = "condetect/tmp"
	tmpUploadPath    = "condetect/tmp/upload"
	uploadPath       = "condetect/uploads"
	downloadPath     = "condetect/download"
	logPath          = "condetect/log"
	dockerLogPath    = "condetect/tmp/docker_logs"
	taskPath         = "condetect/task"
)
...
func (u *DeviceService) Scan() dto.CleanData {
	...
	originalPath := path.Join(global.CONF.System.BaseDir, "condetect_original")
	originalSize, _ := fileOp.GetDirSize(originalPath)
	treeData = append(treeData, dto.CleanTree{
		ID:          uuid.NewString(),
		Label:       "condetect_original",
		Size:        uint64(originalSize),
		IsCheck:     true,
		IsRecommend: true,
		Type:        "condetect_original",
		Children:    loadTreeWithDir(true, "condetect_original", originalPath, fileOp),
	})
...
func (u *DeviceService) Clean(req []dto.Clean) {
	...
	for _, item := range req {
		...
		case "condetect_original":
			dropFileOrDir(path.Join(global.CONF.System.BaseDir, "condetect_original", item.Name))
		...
		case "system_log":
			if len(item.Name) == 0 {
				...
				for _, file := range files {
					if file.Name() == "ConDetect.log" || file.IsDir() {
						continue
					}
...
func (u *DeviceService) CleanForCronjob() (string, error) {
	...
	size := int64(0)
	fileCount := 0
	dropFileOrDirWithLog(path.Join(global.CONF.System.BaseDir, "condetect_original"), &logs, &size, &fileCount)
	...
	if len(logFiles) != 0 {
		for i := 0; i < len(logFiles); i++ {
			if logFiles[i].Name() != "ConDetect.log" {
...
func loadTreeWithAllFile(isCheck bool, originalPath, treeType, pathItem string, fileOp fileUtils.FileOp) []dto.CleanTree {
	...
	for _, file := range files {
		if treeType == "system_log" && (file.Name() == "ConDetect.log" || file.IsDir()) {
			continue
		}
