...
import (
	...
	"ConDetect/backend/utils/copier"
	"ConDetect/backend/utils/xpack"
...
func (u *DashboardService) Restart(operation string) error {
	if operation != "ConDetect" && operation != "system" {
		return fmt.Errorf("handle restart operation %s failed, err: nonsupport such operation", operation)
	}
	itemCmd := fmt.Sprintf("%s condetectctl restart", cmd.SudoHandleCmd())
	if operation == "system" {
