...
import (
	...
	"ConDetect/backend/app/dto/response"
	...
	"ConDetect/backend/utils/ini_conf"
	"ConDetect/backend/utils/systemctl"
	"bytes"
	"fmt"
	"os/exec"
	"os/user"
...
func (h *HostToolService) CreateToolConfig(req request.HostToolCreate) error {
	...
	case constant.Supervisord:
		...
		supervisorDir := path.Join(global.CONF.System.BaseDir, "condetect", "tools", "supervisord")
...
func (h *HostToolService) OperateSupervisorProcess(req request.SupervisorProcessConfig) error {
	var (
		supervisordDir = path.Join(global.CONF.System.BaseDir, "condetect", "tools", "supervisord")
...
func (h *HostToolService) GetSupervisorProcessConfig() ([]response.SupervisorProcessConfig, error) {
	...
	configDir := path.Join(global.CONF.System.BaseDir, "condetect", "tools", "supervisord", "supervisor.d")
...
func (h *HostToolService) OperateSupervisorProcessFile(req request.SupervisorProcessFileReq) (string, error) {
	var (
		...
		configPath = path.Join(global.CONF.System.BaseDir, "condetect", "tools", "supervisord", "supervisor.d", fmt.Sprintf("%s.ini", req.Name))
	)
	switch req.File {
	case "err.log":
...
func operateSupervisorCtl(operate, name, group string) error {
	...
	if name != "" {
		includeDir := path.Join(global.CONF.System.BaseDir, "condetect", "tools", "supervisord", "supervisor.d")
